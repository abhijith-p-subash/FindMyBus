...
export function TripCard({ trip, active = false, compact = false, onOpen, onRemove, style }: TripCardProps) {
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-[7px] min-w-0">
          <div className="flex items-center gap-2">
            ...
            />
            <span className={`font-mono font-medium text-[10px] leading-none tracking-[0.14em] uppercase ${meta.text}`}>
              {meta.label}
              {status === 'idle' && lk ? ` · ${timeAgo(lk.at)}` : ''}
            </span>
          </div>
          <h3 className={`font-display font-semibold leading-tight tracking-tight truncate
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="font-mono font-medium text-[11px] leading-none px-2 py-1.5 rounded-badge
